"""A curated reference database of cities used to seed recommendations.

Every number here is an *indicative, relative approximation* meant for
comparing cities against each other inside this app - it is not live data
and should not be used for financial, safety, or relocation decisions on
its own. Cost-of-living figures loosely use New York City = 100 as a
reference point. See the README for full disclaimers.

Users are never limited to this list: the web app also supports a fully
manual "custom location" path for any place not covered here.
"""
